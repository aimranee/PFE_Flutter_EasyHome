...
class _AddAnnoncePremiumState extends State<AddAnnoncePremium> {
  ...
  String titre, tele, nbrChambre, prix, adresse, description, nbrPersonne, uid;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title:
            Text("Ajouter une annonce", style: TextStyle(color: Colors.white)),
      ),
                    ...
                    onPressed: () async {
                      if (key.currentState.validate()) {
                        ...
                        for (int i = 0; i < images.length; i++) {
                          ...
                          if (urlImage != null) annonce.images.add(urlImage);
                        }
                        if (images.length == annonce.images.length) {
                          bool save = await DBServices().saveAnnonce(annonce);
                          if (save) {
